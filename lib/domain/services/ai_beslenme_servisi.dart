...
// YENİ NESİL HIVE DESTEKLİ, TAMAMEN DİNAMİK BESLENME SERVİSİ (v3.1)
// ============================================================================
...
class AIBeslenmeServisi {
  ...
  Yemek _yemekOlustur(Yemek bazYemek, OgunTipi ogunTipi, double hedefKalori) {
    // 🔥 FIX: Kalori sıfır olsa bile ölçeklemeye çalış, sıfır makro sorununu engelle.
    final olcek = (bazYemek.kalori > 0) ? (hedefKalori / bazYemek.kalori) : 1.0;
    final guvenliOlcek = olcek.clamp(0.5, 2.0);
    ...
    for (final malzemeStr in bazYemek.malzemeler) {
      ...
      if (d.ad.toLowerCase().contains('yumurta')) {
        if (d.birim == 'g') {
          // Gram cinsinden yumurtayı adet'e çevir (1 yumurta ≈ 50-60g, ortalama 55g kullanalım)
          ...
          // Başka birim varsa (kg, ml vs) güvenlik için olduğu gibi bırak
          AppLogger.warning('🛡️ YUMURTA GÜVENLİK: "${d.ad}" bilinmeyen birim (${d.birim}), ölçek dışı bırakıldı.');
          yeniMalzemeler.add(malzemeStr);
          continue;
        }
        // Eğer zaten adet ise, normal ölçekleme devam edecek
      }
  ...
  Yemek _enUygunYemekSecHive(
    ...
    for (final yemek in filtrelenmisYemekler) {
      ...
      if (yemek.ogun != ogunTipi) {
        // Eğer bir ana öğün yerine başka bir ana öğün geliyorsa hafif ceza,
        // ama bir ana öğün yerine ara öğün geliyorsa ÇOK AĞIR ceza.
        ...
        if (hedefAnaOgun != yemekAnaOgun) {
          cezaPuani *= 50.0; // Ana öğün yerine ara öğün (veya tam tersi) gelmesini engelle
        } else {
          cezaPuani *= 5.0; // Öğle yemeği yerine akşam yemeği gelmesi gibi durumlar için
        }
...
extension AIBeslenmeServisiTestExtension on AIBeslenmeServisi {
