...
void main() {
  ...
  group('FAZ 4 - Yemek Entity ve JSON Parser Testleri', () {
    ...
    test('Tüm öğünleri paralel yükle', () async {
      ...
      AppLogger.success('✅ $toplamYemek yemek ${stopwatch.elapsedMilliseconds}ms\'de yüklendi');
    ...
    test('Yemek makro uygunluğu kontrolü', () async {
      ...
      final uygunYemekler = kahvaltilar
          .where((y) => y.makroyaUygunMu(hedefler, 0.2))
          .toList();
      
      // Assert
      expect(uygunYemekler, isNotEmpty);
      
      AppLogger.success('✅ ${uygunYemekler.length}/${kahvaltilar.length} yemek makrolara uygun');
    ...
    test('Alerji kısıtlaması ile filtreleme', () async {
      ...
      AppLogger.debug('Kısıtlamasız yemekler: ${filtrelenmis.map((y) => y.ad).join(", ")}');
    });
    ...
    test('Kalori aralığına göre filtreleme', () async {
      ...
      AppLogger.debug('$minKalori-$maxKalori kcal aralığında ${filtrelenmis.length} yemek');
    });
    ...
    test('Alternatif besin önerileri kontrolü', () async {
      ...
      // Act
      final alternatifliYemekler = kahvaltilar
          .where((y) => y.alternatifler.isNotEmpty)
          .toList();
      
      // Assert
      if (alternatifliYemekler.isNotEmpty) {
        AppLogger.success('✅ ${alternatifliYemekler.length} yemekte alternatif var');
        
        for (final yemek in alternatifliYemekler) {
          AppLogger.debug('${yemek.ad}:');
          for (final alt in yemek.alternatifler) {
            AppLogger.debug('  - ${alt.orijinalBesin} -> ${alt.alternatifler.length} alternatif');
          }
        }
      }
    });
    ...
    test('FAZ 4 - GENEL PERFORMANS TESTİ', () async {
      ...
      AppLogger.info('  • Ortalama protein: ${ortalamaProtein.toStringAsFixed(0)}g');
      AppLogger.info('  • Yükleme süresi: ${yuklemeZamani}ms');
      AppLogger.info('  • Toplam test süresi: ${stopwatch.elapsedMilliseconds}ms');
      AppLogger.info('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
